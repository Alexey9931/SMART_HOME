...
void DrawSun()
{
	...
	for (int k = 0; k < 10; k++)
	{
		...
		for (int m = 0; m < 1000; m++)
		{
			...
			ILI9486_drawPixel(370+m%100, 3+10*k+(m/100), (uint16_t)strtol(arr,NULL,16));
			wdt_reset();
		}
	}
	pf_mount(0x00);
...
void DrawLevelNrf()
{
	...
	for (int k = 0; k < 10; k++)
	{
		...
		for (int m = 0; m < 90; m++)
		{
			...
			p += 2;
			ILI9486_drawPixel(280+m%30, 10+3*k+(m/30), (uint16_t)strtol(arr,NULL,16));
			wdt_reset();
		}
	}
	pf_mount(0x00);
