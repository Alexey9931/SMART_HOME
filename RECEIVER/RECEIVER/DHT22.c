...
uint8_t dht22_GetData (uint8_t *data)
{
	...
	for (j=0; j<5; j++)
	{
		...
		for (i = 0; i<8; i++)
		{
			while(!(PINK & (1 << DHT22_PORT)));//ждем отпускани€ шины
			_delay_us(35);
			if (PINK & (1 << DHT22_PORT))//читаем результат по прошествии 30 микросекунд
			//если шина за это врем€ не прит€нулась к земле, то значит это единица, иначе ноль
			data[4-j] |= (1<<(7-i));
			while(PINK & (1 << DHT22_PORT));//ждем пока датчик прит€нет шину(в случае единицы)
		}
	}
	return 1;
}